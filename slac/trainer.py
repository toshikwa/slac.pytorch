...
class Trainer:
    ...
    def __init__(
        ...
        # Input stores for training and evaluation.
        self.input = SlacInput(env.observation_space.shape, env.action_space.shape, num_sequences)
        self.input_test = SlacInput(env.observation_space.shape, env.action_space.shape, num_sequences)
    ...
    def train(self):
        ...
        for step in range(1, self.initial_collection_steps + 1):
            t = self.algo.step(self.env, self.input, t, step <= self.initial_collection_steps)
        ...
        for step in range(self.initial_collection_steps + 1, self.num_steps // self.action_repeat + 1):
            t = self.algo.step(self.env, self.input, t, False)
    ...
    def evaluate(self, step_env):
        ...
        for i in range(self.num_eval_episodes):
            ...
            while not done:
                action = self.algo.exploit(self.input_test)
